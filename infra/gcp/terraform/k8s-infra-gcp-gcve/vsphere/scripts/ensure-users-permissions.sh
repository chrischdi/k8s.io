...
govc permissions.ls / | grep ${GCVE_PROW_CI_GROUP} | grep -q cns-searchable || (govc permissions.set -propagate=false -group -principal GVE.LOCAL\\${GCVE_PROW_CI_GROUP} -role cns-searchable / && echo "Added permissions for ${GCVE_PROW_CI_GROUP} as cns-searchable to /")
